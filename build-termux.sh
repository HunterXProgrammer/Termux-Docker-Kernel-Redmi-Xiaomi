...
bash "$tmpfile" install --override-alias $distro-kernel-builder $distro 2>&1 | grep -Ev "Now run.*to log in"

bash $tmpfile login --isolated --termux-home $distro-kernel-builder -- bash -c "apt update --allow-insecure-repositories 2>/dev/null && apt install --allow-unauthenticated -y git curl python2 gcc gcc-arm-linux-gnueabi make zip bc libssl-dev xz-utils diffutils 2>&1 | grep -v -e warning -e WARNING -e Readline -e Dialog && cd \"\$HOME/$dir_name\" && bash build.sh termux"
